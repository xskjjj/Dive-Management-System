Feature: Register Member (p9)
  ...
  Scenario Outline: Register a member account successfully
    When a new member attempts to register with "<email>" , "<password>" , "<name>", "<emergencyContact>", "<numDays>", "<items>", "<requestedQuantities>", "<guideRequired>", and "<hotelRequired>" (p9)
    Then a new member account shall exist with "<email>", "<password>", "<name>", "<emergencyContact>", "<numDays>", "<items>", "<requestedQuantities>", "<guideRequired>", and "<hotelRequired>" (p9)
  ...
  Scenario Outline: Register member account failed
    ...
    Examples: 
      | email          | password  | name  | emergencyContact | numDays | items  | requestedQuantities | guideRequired | hotelRequired | error                                                                                                                       |
      ...
      | user@mail.ca   | password  | User  | (555)555-5555    |       0 | rope           |                   3 | false         | true          | The number of days must be greater than zero and less than or equal to the number of diving days in the diving season |
      | user@mail.ca   | password  | User  | (555)555-5555    |      21 | rope           |                   3 | true          | false         | The number of days must be greater than zero and less than or equal to the number of diving days in the diving season |
      | admin@ad.atl   | password  | User  | (555)555-5555    |       5 | rope           |                   3 | true          | false         | The email entered is not allowed for members                                                                                |
